#!/usr/bin/env python3
"""
Regenerates index.html — a live FBR stale-ticket report — from Jira data.
...
def jql_search(jql, fields=None, max_results=100):
...
def build_html(stale, total_open, in_progress, created_today, done_today, resolved_7d):
    # per-assignee breakdown of the stale set
    counts = {}
    for issue in stale:
    ...
    table_rows = ""
    for issue in sorted(stale, key=lambda i: i["fields"]["created"]):
        key = issue["key"]
        f = issue["fields"]
        table_rows += f"""
      <tr>
...
<style>
  body {{ font-family: -apple-system, Segoe UI, Arial, sans-serif; max-width: 860px; margin: 40px auto; padding: 0 20px; color: #1a1a1a; background: #fff; }}
...
<body>
  ...
  <h2>Stale backlog by assignee ({len(stale)} total)</h2>
  {bar_rows}

  <h2>Stale ticket detail</h2>
  <table>
...
def main():
    ...
    html = build_html(stale, total_open, in_progress, created_today, done_today, resolved_7d)
    with open("index.html", "w") as f:
        f.write(html)
    print(f"Wrote index.html — {len(stale)} stale tickets, {total_open} total open.")
